# -*- coding: utf-8 -*-
'''
    Created by hushiwei on 2018/6/30
    Desc : 特征工程
'''
...
# df = pd.read_csv('./datas/merge/process01_000', header=None, names=['from', 'to', 'date', 'content', 'label'])

df = pd.read_csv('./datas/result_process01', header=None, names=['from', 'to', 'date', 'content', 'label'])


def getEmailAddress(strl):
...
df['from_address'] = pd.Series(map(lambda str: getEmailAddress(str), df['from']))

df['to_address'] = pd.Series(map(lambda str: getEmailAddress(str), df['to']))

print("-" * 20, "to address", "-" * 20)
...
def getDateFeatureByDateLength(str1):
    '''
...
date_time_extract_result = list(map(lambda st: getDateFeatureByDateLength(st), df['date']))
...
df['content_length_type'] = pd.Series(map(lambda st: staticByEmailLength(st), df['content_length']))
...
df.to_csv('./datas/result_process02.csv', encoding='utf-8', index=False)
