...
song = AudioSegment.from_file('./datas/3D音效音乐/天空之城.MP3', format='mp3')  # 读入歌曲
# song_split = song[-30*1000 :]#切分歌曲
song.export('./datas/3D音效音乐/天空之城.wav', format='wav')  # MP3到wav的转换
rate, data = wavfile.read('./datas/3D音效音乐/天空之城.wav')  # 每秒播放速度及数据
...
# print(mf_feat)
# print(mf_feat.shape)
mm = np.mean(mf_feat, axis=0)  # 按行求均值,返回一行n列了. 隐含了时域上的相关性
