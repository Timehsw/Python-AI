...
def random_sample_playlist(in_file, out_file, max_sample=10, seed=None):
    """
    歌单数据随机抽样，输入的数据中一行一个歌单，输出也是一行一个歌单
    ...
    with open(out_file, 'w', encoding='utf-8') as writer:
        with open(in_file, 'r', encoding='utf-8') as reader:
            ...
            for line in reader:
                ...
                if count >= max_sample:
                    # 当抽取的样本数目达到给定的限制值的时候，结束数据的抽取操作
                    break
            print("实际抽取的总歌单数量:%d" % count)


# 将JSON格式的歌单数据转换为定义格式并返回的函数API
...
#  将JSON格式的歌单文件数据转换为定义格式并输出到磁盘文件的函数的API
def parse_playlist_file(in_file, out_file):
    ...
    with open(out_file, 'w', encoding='utf-8') as writer:
        with open(in_file, 'r', encoding='utf-8') as reader:
            for line in reader:
                # reader中的数据每行一个歌单信息，所以这里需要对line歌单信息进行处理后，结果再保存到文件中
...
# 将定义格式的数据转换为用户-歌曲评分信息并输出到磁盘文件的函数API
...
# 将定义格式的数据转换为歌单-歌曲评分信息并输出到磁盘文件的函数API
...
if __name__ == '__main__':
    # 定义一些数据文件路径
    # 总歌单数大概是3万多个歌单
    all_song_file_path = "../datas/playlist_detail_music_500.json"
    sample_song_file_path = '../datas/playlist_detail_music_99.json'
    music_playlist_song_file_path = '../datas/163_music_playlist.txt'
    music_user_song_rating_file_path = '../datas/163_music_user_song_rating.txt'
    music_playlist_song_rating_file_path = '../datas/163_music_playlist_song_rating.txt'
    playlist_pkl_file_path = "../datas/playlist.pkl"
    song_pkl_file_path = "../datas/song.pkl"

    # 1. 样本数据抽取
    # random_sample_playlist(all_song_file_path, sample_song_file_path, 500)

    # 2. 特征属性的提取
    parse_playlist_file(sample_song_file_path, music_playlist_song_file_path)
