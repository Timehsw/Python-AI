...
folderOfData = '/Users/ryanzheng/PycharmProjects/data/'
data1 = pd.read_csv(folderOfData+'xxxx.csv', header = 0)
data2 = pd.read_csv(folderOfData+'xxxx.csv', header = 0,encoding = 'gbk')
data3 = pd.read_csv(folderOfData+'xxxx.csv', header = 0)

#############################################################################################
print(' Step 1: 从data1 &  data3数据中衍生特征')
#############################################################################################
...
trainData['target'] = trainData['target'].astype(np.int64)
print(
'''
对于类别型变量，按照以下方式处理
...
var_WOE = {}
for col in categorical_var:#col = 'WeblogInfo_2'
    ...
    if len(set(trainData[col]))>5:
        ...
        #(1), 计算坏样本率并进行编码
        encoding_result = dev_fuc.BadRateEncoding(trainData, col, 'target')
        ...
        if min(bad_bin) == 0:
            ...
            mergeBin = dev_fuc.MergeBad0(trainData, col, 'target')
            #(2), 依照（1）的结果对值进行合并
            trainData[col1] = trainData[col].map(mergeBin)
            maxPcnt = dev_fuc.MaximumBinPcnt(trainData, col1)
            #如果合并后导致有箱占比超过90%，就删除。
            if maxPcnt > 0.9:
            ...
            WOE_IV = dev_fuc.CalcWOE(trainData, col1, 'target')
            var_WOE[col1] = WOE_IV['WOE']
            var_IV[col1] = WOE_IV['IV']
            #del trainData[col]
            deleted_features.append(col)
        else:
            WOE_IV = dev_fuc.CalcWOE(trainData, col, 'target')
...
for col in numerical_var:
    ...
    if -1 in set(trainData[col]):
        ...
        special_attribute = []
    cutOffPoints = dev_fuc.ChiMerge(trainData, col, 'target',special_attribute=special_attribute)
    var_cutoff[col] = cutOffPoints
    trainData[col1] = trainData[col].map(lambda x: dev_fuc.AssignBin(x, cutOffPoints,special_attribute=special_attribute))

    #(2), check whether the bad rate is monotone
    BRM = dev_fuc.BadRateMonotone(trainData, col1, 'target',special_attribute=special_attribute)
    if not BRM:
        if special_attribute == []:
            bin_merged = dev_fuc.Monotone_Merge(trainData, 'target', col1)
            removed_index = []
            ...
            bin_merged = dev_fuc.Monotone_Merge(temp, 'target', col1)
            removed_index = []
            for bin in bin_merged:
                if len(bin) > 1:
                    indices = [int(b.replace('Bin ', '')) for b in bin]
            ...
            trainData[col1] = trainData[col].map(lambda x: dev_fuc.AssignBin(x, cutOffPoints2, special_attribute=special_attribute))
...
########################################
print(' Step 4: WOE编码后的单变量分析与多变量分析')
########################################
...
num2str = ['SocialNetwork_13','SocialNetwork_12','UserInfo_6','UserInfo_5','UserInfo_10','UserInfo_11','UserInfo_12','UserInfo_13','UserInfo_17']
for col in num2str:
...
for col in var_WOE.keys():
    ...
    if col in var_cutoff.keys():
        ...
        if - 1 in cutOffPoints:
            special_attribute = [-1]
        binValue = trainData[col].map(lambda x: dev_fuc.AssignBin(x, cutOffPoints,special_attribute=special_attribute))
...
iv_threshould = 0.02
varByIV = [k for k, v in var_IV.items() if v > iv_threshould and v < 10]



print('(ii) 检查WOE编码后的变量的两两线性相关性')
...
print('(iii）检查是否有变量与其他所有变量的VIF > 10')
...
dev_fuc.KS(y_result,'y_pred','y_real')
