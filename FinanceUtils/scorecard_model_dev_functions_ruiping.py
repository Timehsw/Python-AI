...
def CalcWOE(df, col, target):
    ...
    #regroup['WOE'] = regroup.apply(lambda x: np.log(x.good_pcnt*1.0/(x.bad_pcnt+1e-10),axis = 1))
    regroup['WOE'] = np.log(regroup['good_pcnt']*1.0/(regroup['bad_pcnt']+1e-10))
    WOE_dict = regroup[[col,'WOE']].set_index(col).to_dict(orient='index')
    for k, v in WOE_dict.items():
        WOE_dict[k] = v['WOE']
    IV = regroup.apply(lambda x: (x.good_pcnt-x.bad_pcnt)*np.log(x.good_pcnt*1.0/(x.bad_pcnt+1e-10)),axis = 1)
    IV = sum(IV)
