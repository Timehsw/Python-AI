...
'''
    ...
    Desc : 卷积神经网络可视化,直观理解每一层卷积对图像有哪些影响
    Note : https://blog.csdn.net/u013719780/article/details/53907105
'''
import sys
import numpy as np
...
def conv_model(_input, _w, _b):
    ...
    # Vectorize
    _dense = tf.reshape(_pool, shape=[-1, _w['wd1'].get_shape().as_list()[0]])
...
if do_train == 1:
    for epoch in range(traing_epochs):
        ...
        for i in range(total_batch):
            ...
            # Compute averate loss
            avg_cost += sess.run(cost, feed_dict={x: batch_xs, y: batch_ys}) / total_batch

        # Display logs per epoch step
        if epoch % display_step == 0:
            print('Epoch : %03d/%03d cost: %.9f' % (epoch, traing_epochs, avg_cost))
