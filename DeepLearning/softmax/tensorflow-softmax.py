...
# 2. 模型构建
# 构建数据输入占位符x和y
# x/y:None的意思表示维度未知(也就是我们可以传入任意的数据样本条数)
...
# tf.equal : 是对比这两个矩阵或者向量的相等的元素,如果是相等的那就返回True,否则返回False.返回的值的矩阵维度和A是一样的
...
with tf.Session() as session:
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(num_batch):
            # 获取传入进行模型训练的数据对应索引 ,从打乱了顺序的索引中取出10个索引
            ...
            # 可选:获取损失函数值
            avg_cost += session.run(cost, feed_dict=feeds) / num_batch

        if epoch % display_step == 0:
            feeds_train = {x: x_data, y: y_data}
            train_acc = session.run(acc, feed_dict=feeds)
            print('迭代次数,%03d/%03d 损失值: %.9f 训练集上准确率:%.3f' % (epoch, training_epochs, avg_cost, train_acc))

    # 对用于画图的数据进行预测
    # y_hat: 是一个None*2的矩阵
    y_hat = session.run(act, feed_dict={x: x_test})
