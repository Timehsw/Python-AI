...
from sklearn.naive_bayes import MultinomialNB, BernoulliNB  # 多项式和伯努利贝叶斯
...
# 完成
print('完成数据加载过程,耗时:%.3fs' % (time.time() - t_start))
...
print('数据类型: ', type(data_train.data))
...
print('\n----- 前5个样本 -----\n')
for i in range(5):
    print('文本%d(所属类别 - %s):' % (i + 1, categories[y_train[i]]))
    print(x_train[i])
    print('\n\n')
...
def benchmark(clf, name):
    ...
    print('5折交叉验证的训练时间为:%.3fs/(5*%d)=%.3fs' % ((t_end - t_start), m, t_train))
    ...
    # 返回结果(训练时间耗时,预测时间耗时,训练数据错误率,测试数据错误率,名称)
...
clfs = [
    ...
    [LinearSVC(loss='squared_hinge', penalty='l2', dual=False, tol=1e-4), 'LinearSVC-l1']
]
